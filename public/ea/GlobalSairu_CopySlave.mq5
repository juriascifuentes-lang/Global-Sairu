...
void ProcessSignals() {
   ...
   string url = SUPABASE_URL + "/rest/v1/rpc/get_pending_signals";
   ...
   string rpc_body = StringFormat("{\"p_master_name\":\"%s\",\"p_slave_name\":\"%s\"}",
                                   g_master_name, SLAVE_NAME);
   char req[], res[];
   string res_headers;
   StringToCharArray(rpc_body, req, 0, StringLen(rpc_body));
   ...
   while(true) {
      ...
      if(!g_initialized) {
         // Durante init: marcar todas las señales pendientes como ignoradas.
         // El SYNC se encarga de abrir lo que corresponde según el estado real del maestro.
...
void MarkProcessed(long sig_id, long exec_ticket, string exec_comment) {
   string url     = SUPABASE_URL + "/rest/v1/copy_signal_acks";
   string headers = "Content-Type: application/json\r\n"
                  + "apikey: " + SUPABASE_KEY + "\r\n"
                  + "Authorization: Bearer " + SUPABASE_KEY + "\r\n"
   ...
   string body = StringFormat(
      "{\"signal_id\":%d,\"slave_name\":\"%s\",\"exec_ticket\":%d,"
      "\"exec_comment\":\"%s\",\"slave_balance\":%s}",
      sig_id, SLAVE_NAME, exec_ticket, exec_comment, slave_bal);
   ...
   if(code != 200 && code != 201 && code != 204)
      PrintFormat("[GS-Slave] ERROR ACK sig_id=%d code=%d", sig_id, code);
}
...
void InitDayStart() {
   ...
   g_day_start_time   = StructToTime(dt);
   PrintFormat("[GS-Slave] Día iniciado: %04d-%02d-%02d desde %s",
               dt.year, dt.mon, dt.day, TimeToString(g_day_start_time));
}

//── P&L real del día: deals cerrados + flotante ──────────────────────
double CalcDayPnL() {
   HistorySelect(g_day_start_time, TimeCurrent() + 1);
   double pnl = 0.0;
...
void ReportBalance() {
   ...
   if(dt.day_of_year != g_current_day) {
      ...
      g_day_start_time = StructToTime(midnight);
   }
   ...
   string body = StringFormat(
      ...
      SLAVE_NAME, balance, equity, profit, day_profit, today);
   char req[], res[];
   string res_headers;
   StringToCharArray(body, req, 0, StringLen(body));
   int bal_code = WebRequest("POST", url, headers, 5000, req, res, res_headers);
   if(bal_code != 200 && bal_code != 201 && bal_code != 204)
      PrintFormat("[GS-Slave] ERROR balance HTTP %d: %s", bal_code, CharArrayToString(res));
}
...
void SyncPositionsOnInit() {
   ...
   while(true) {
      ...
      if(stype == "OPEN" && cnt < 500) {
         ...
         master_bals[cnt]  = ParseDouble(obj, "master_balance");
         cnt++;
   ...
   for(int i = 0; i < cnt; i++) {
      ...
      if(!has_pos) {
         ...
         double price = (ot == ORDER_TYPE_BUY)
            ...
            : SymbolInfoDouble(slave_sym, SYMBOL_BID);
         if(price > 0.0) {
            double slave_lots = CalcLots(lots_a[i], master_bals[i], slave_sym);
